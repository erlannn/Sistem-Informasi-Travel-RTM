...
<div class="max-w-3xl mx-auto space-y-8">
    ...
    <div class="flex items-center justify-between bg-white p-6 sm:p-8 rounded-3xl border border-slate-200 shadow-sm">
        <div>
            ...
            <p class="text-xs text-slate-500 font-medium">Atur rute, jam keberangkatan, serta penyesuaian harga tiket & gaji sopir secara fleksibel.</p>
        </div>
        ...
        <form action="{{ route('admin.jadwal.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select id="select-asal" name="asal" required
                        ...
                        <option value="Solok" {{ old('asal') == 'Solok' ? 'selected' : '' }}>Solok</option>
                ...
                <div>
                    ...
                    <select id="select-tujuan" name="tujuan" required
                        ...
                        <option value="Solok" {{ old('tujuan') == 'Solok' ? 'selected' : '' }}>Solok</option>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" id="input-harga" name="harga" value="{{ old('harga', '80000') }}" min="0" step="1000" required
                ...
                <div>
                    ...
                    <div class="relative">
                        <span class="absolute inset-y-0 left-0 flex items-center pl-3.5 text-xs font-black text-slate-400">Rp</span>
                        <input type="number" id="input-gaji-sopir" name="bagi_hasil_sopir" value="{{ old('bagi_hasil_sopir', '30000') }}" min="0" step="1000" required
                            class="w-full pl-10 pr-4 py-3 text-xs font-extrabold text-slate-900 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-amber-400 focus:border-amber-400 transition outline-none">
                    </div>
            ...
            <div id="route-info-banner" class="bg-amber-50 border border-amber-200 p-4 rounded-2xl space-y-1 text-xs">
                <div class="flex justify-between font-extrabold text-slate-900">
                    <span id="route-title">Sijunjung &rarr; Padang</span>
                    <span id="route-price" class="text-amber-700 font-black">Rp 80.000 / Tiket</span>
                </div>
                <div class="flex justify-between text-[11px] text-slate-600 font-medium pt-1.5 border-t border-amber-200/60">
                    <span>Gaji Sopir (Per Tiket): <strong id="route-driver" class="text-emerald-700">Rp 30.000</strong></span>
                    <span>Setoran Perusahaan (Per Tiket): <strong id="route-company" class="text-blue-700">Rp 50.000</strong></span>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const defaultPricing = {
        'Sijunjung-Solok': { harga: 50000, supir: 20000 },
        'Solok-Sijunjung': { harga: 50000, supir: 20000 },

        'Sijunjung-Padang': { harga: 80000, supir: 30000 },
        'Padang-Sijunjung': { harga: 80000, supir: 30000 },

        'Sijunjung-BIM': { harga: 150000, supir: 50000 },
        'BIM-Sijunjung': { harga: 150000, supir: 50000 },

        'Padang-Solok': { harga: 60000, supir: 25000 },
        'Solok-Padang': { harga: 60000, supir: 25000 },

        'BIM-Solok': { harga: 70000, supir: 30000 },
        'Solok-BIM': { harga: 70000, supir: 30000 },

        'Padang-BIM': { harga: 40000, supir: 15000 },
        'BIM-Padang': { harga: 40000, supir: 15000 },
    };
    ...
    function onRouteChanged(e) {
        ...
        // Auto suggest prices if key matches default pricing
        const key = `${asalSelect.value}-${tujuanSelect.value}`;
        if (defaultPricing[key]) {
            hargaInput.value = defaultPricing[key].harga;
            gajiSopirInput.value = defaultPricing[key].supir;
        }
