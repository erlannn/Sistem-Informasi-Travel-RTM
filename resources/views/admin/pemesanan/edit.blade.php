...
@section('title', 'Edit Pemesanan - CV Travel RTM')
@section('page_title', 'Edit Transaksi Pemesanan Tiket')

@section('content')
<div class="max-w-3xl mx-auto space-y-8" x-data="{
    selectedJadwalId: '{{ old('id_jadwal', $pemesanan->id_jadwal) }}',
    selectedKursiId: '{{ old('id_kursi', $pemesanan->id_kursi) }}',
    ...
    <div class="flex items-center justify-between bg-white p-6 sm:p-8 rounded-3xl border border-slate-200 shadow-sm">
        <div>
            <span class="px-3.5 py-1 rounded-full bg-amber-100 border border-amber-300 text-amber-900 text-xs font-black uppercase tracking-wider">
                Edit Transaksi #{{ $pemesanan->id_pemesanan }}
            </span>
            <h1 class="text-xl sm:text-2xl font-black text-black tracking-tight mt-2">
                Edit Transaksi Pemesanan Tiket
            </h1>
            <p class="text-xs sm:text-sm text-black font-medium mt-1 leading-relaxed">Ubah detail penumpang, rute jadwal, nomor kursi, atau status transaksi.</p>
        </div>
    ...
    <div class="bg-white rounded-3xl p-6 sm:p-8 md:p-10 border border-slate-200 shadow-sm">
        <form action="{{ route('admin.pemesanan.update', $pemesanan->id_pemesanan) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="id_penumpang" required
                    ...
                    @foreach($penumpangs as $pn)
                        <option value="{{ $pn->id_penumpang }}" {{ old('id_penumpang', $pemesanan->id_penumpang) == $pn->id_penumpang ? 'selected' : '' }}>
            ...
            <div>
                <label class="block text-xs sm:text-sm font-black uppercase tracking-wider text-black mb-2">
                    Pilih Nomor Kursi <span class="text-red-500">*</span>
                </label>
                        ...
                        <optgroup label="Kursi {{ $j->asal }} &rarr; {{ $j->tujuan }} ({{ $j->tanggal }})" x-show="selectedJadwalId == '{{ $j->id_jadwal }}'">
                            @foreach($j->kursis as $k)
                                <option value="{{ $k->id_kursi }}"
                                    x-show="'{{ $k->status }}' === 'Tersedia' || '{{ $k->id_kursi }}' == '{{ $pemesanan->id_kursi }}'"
                                    :disabled="'{{ $k->status }}' !== 'Tersedia' && '{{ $k->id_kursi }}' != '{{ $pemesanan->id_kursi }}'">
                                    Kursi {{ $k->nomor_kursi }} @if($k->status !== 'Tersedia' && $k->id_kursi != $pemesanan->id_kursi) ({{ $k->status }}) @endif
                                </option>
                            @endforeach
                        </optgroup>
                    @endforeach
            ...
            <div>
                ...
                <select name="status" required
                    class="w-full px-4 py-3.5 text-sm font-bold text-black bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-amber-400 focus:border-amber-400 transition outline-none">
                    <option value="Pending" {{ old('status', $pemesanan->status) == 'Pending' ? 'selected' : '' }}>Pending (Menunggu Bayar)</option>
                    <option value="Lunas" {{ old('status', $pemesanan->status) == 'Lunas' ? 'selected' : '' }}>Lunas (Telah Dibayar)</option>
                    <option value="Batal" {{ old('status', $pemesanan->status) == 'Batal' ? 'selected' : '' }}>Batal</option>
                </select>
                @error('status')
                    <p class="text-xs text-red-500 font-bold mt-1.5">{{ $message }}</p>
                @enderror
