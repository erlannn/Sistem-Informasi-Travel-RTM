...
<div class="space-y-6">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-white p-6 rounded-3xl border border-slate-200/80 shadow-xs">
        <div>
            ...
            <p class="text-xs text-slate-500 font-medium mt-0.5">Manajemen unit kendaraan, kapasitas kursi, dan status operasional armada</p>
        </div>
        ...
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            ...
            <form action="{{ route('admin.armada.index') }}" method="GET" class="flex items-center gap-2 w-full sm:w-auto">
                ...
                @if(request()->filled('search'))
                    <a href="{{ route('admin.armada.index') }}" class="px-3.5 py-2.5 bg-slate-100 hover:bg-slate-200 active:scale-95 text-slate-700 font-bold text-xs rounded-xl transition-all cursor-pointer shrink-0">
                        Reset
                    </a>
                @endif
            </form>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-100/80 text-slate-700 uppercase text-[11px] font-extrabold tracking-wider border-b border-slate-200">
                        ...
                        <th class="py-3.5 px-4">Merk Armada</th>
                        <th class="py-3.5 px-4">Warna</th>
                        <th class="py-3.5 px-4">Kapasitas</th>
                        <th class="py-3.5 px-4">Total Perjalanan</th>
                        <th class="py-3.5 px-4 text-center">Status Operasional</th>
                ...
                <tbody class="divide-y divide-slate-100 text-xs md:text-sm">
                    @forelse($armadas as $a)
                        <tr class="hover:bg-slate-50/80 transition-colors">
                            ...
                            <!-- Kapasitas (Tanpa Ikon) -->
                            <td class="py-4 px-4 font-bold text-slate-800">
                                {{ $a->kursi }} Kursi
                            </td>

                            <!-- Total Perjalanan (Tanpa Ikon) -->
                            <td class="py-4 px-4 font-bold text-slate-800">
                                {{ $a->jadwals_count }} Jadwal
                            </td>

                            <!-- Status Operasional Badge (Struktur Blade Lengkap) -->
                        ...
                        <tr>
                            <td colspan="7" class="py-12 text-center text-slate-500 font-medium text-xs sm:text-sm">
                                <div class="flex flex-col items-center justify-center gap-2">
                                    <i class="fa-solid fa-van-shuttle text-3xl text-slate-300"></i>
                                    <p class="font-bold text-slate-700">Belum ada data armada travel di database.</p>
                                    <p class="text-xs text-slate-400">Klik tombol "+ Tambah Armada" di atas untuk menambahkan unit kendaraan baru.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
