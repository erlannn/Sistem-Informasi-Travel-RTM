...
@section('title', 'Detail Pemesanan RTM0001 - RTM Family')
...
<div class="py-8 bg-slate-50 md:py-12 print-container">
    <div class="px-4 mx-auto max-w-3xl sm:px-6 lg:px-8">
        ...
        <!-- Ticket Voucher Card (Designed like a boarding pass with cutouts) -->
        <div class="ticket-voucher bg-white rounded-3xl border border-slate-200/80 shadow-md overflow-hidden relative mb-8 transition-all hover:shadow-lg">
            ...
            <!-- Side Cutout Circles (Ticket Aesthetic) -->
                    ...
                    <div class="space-y-3">
                        <div class="flex items-center gap-4">
                            <div class="flex flex-col">
                                ...
                                <span class="text-base font-extrabold text-slate-900">{{ request('asal', 'Sijunjung') }}</span>
                            </div>
                            <!-- Arrow Symbol -->
                            <div class="text-gold-500 font-bold text-lg">&rarr;</div>
                            <div class="flex flex-col">
                                <span class="text-xs font-bold text-slate-400">Ke</span>
                                <span class="text-base font-extrabold text-slate-900">{{ request('tujuan', 'Padang') }}</span>
                            </div>
                        </div>
                        <div class="grid grid-cols-2 gap-4 pt-2">
                            <div>
                                <span class="text-xs text-slate-400 block">Tanggal</span>
                                <span class="text-sm font-bold text-slate-800">{{ request('tanggal', '20 Juli 2026') }}</span>
                            </div>
                            <div>
                                <span class="text-xs text-slate-400 block">Jam Berangkat</span>
                                <span class="text-sm font-bold text-slate-800">{{ request('jam', '05.00 WIB') }}</span>
                            </div>
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <span class="text-xs text-slate-400 block">Armada</span>
                                <span class="text-sm font-bold text-slate-800">{{ request('armada', 'Toyota Avanza') }}</span>
                            </div>
                            <div>
                                <span class="text-xs text-slate-400 block">No. Kursi</span>
                                <span class="text-sm font-bold text-gold-600">Kursi {{ request('kursi', '1') }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-4 md:border-l md:border-slate-100 md:pl-6">
                    ...
                    <div class="space-y-3">
                        <div>
                            ...
                            <span class="text-sm font-bold text-slate-800">{{ request('nama', 'Sri Erma Novira') }}</span>
                        </div>
                        <div>
                            <span class="text-xs text-slate-400 block">Alamat Email</span>
                            <span class="text-sm font-semibold text-slate-600">{{ request('email', 'srierma@email.com') }}</span>
                        </div>
                        <div>
                            <span class="text-xs text-slate-400 block">Nomor Telepon</span>
                            <span class="text-sm font-semibold text-slate-600">{{ request('telepon', '081234567890') }}</span>
                        </div>
                    </div>
                </div>
            ...
            <div class="p-6 md:p-8 bg-slate-50/50 grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
                ...
                <div class="md:col-span-7 space-y-3">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <div class="flex justify-between items-center text-sm font-bold text-slate-900 border-t border-dashed border-slate-200 pt-2">
                            ...
                            <span class="text-base text-gold-600">Rp 70.000</span>
                        </div>
                    </div>
                    <!-- Small note -->
                ...
                <!-- Digital Ticket Barcode concept (5 Cols - UX Correction: Printable ticket barcode block) -->
                <div class="md:col-span-5 flex flex-col items-center justify-center border-t md:border-t-0 md:border-l border-slate-200/80 pt-6 md:pt-0 md:pl-6">
                    ...
                    <div class="bg-white p-3.5 rounded-xl border border-slate-200/60 shadow-xs flex flex-col items-center gap-1.5">
                        ...
                        <span class="text-[9px] font-mono tracking-[0.25em] text-slate-500 font-bold">RTM0001</span>
                    </div>
        ...
        <div class="flex flex-col sm:flex-row items-center justify-center gap-4 no-print">
            <a href="/booking/status" class="w-full sm:w-auto px-6 py-3 text-sm font-semibold text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-xl transition-colors text-center cursor-pointer">
                Kembali
            </a>
