...
<div class="py-8 bg-slate-50 md:py-12">
    <div class="px-4 mx-auto max-w-5xl sm:px-6 lg:px-8">
        ...
        <form action="/booking/status-detail" method="GET" id="booking-confirmation-form">
            <!-- Propagate travel info query tags to the invoice page -->
            <input type="hidden" name="kode" value="RTM0001">
            <input type="hidden" name="asal" value="{{ request('asal', 'Sijunjung') }}">
            <input type="hidden" name="tujuan" value="{{ request('tujuan', 'Padang') }}">
            <input type="hidden" name="tanggal" value="{{ request('tanggal', '20 Juli 2026') }}">
            <input type="hidden" name="jam" value="{{ request('jam', '05.00 WIB') }}">
            <input type="hidden" name="armada" value="{{ request('armada', 'Toyota Avanza') }}">
            <input type="hidden" name="kursi" value="{{ request('kursi', '1') }}">
                    ...
                    <div class="bg-white rounded-2xl shadow-card border border-slate-100 p-6 md:p-8 transition-all">
                        ...
                        <div class="space-y-5">
                            ...
                            <div>
                                ...
                                <input type="text" id="nama" name="nama" required value="Sri Erma Novira" class="block w-full px-4 py-3 text-sm text-slate-800 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-gold-500/20 focus:border-gold-500 transition-all">
                            </div>

                            <!-- Input: Email -->
                            <div>
                                <label for="email" class="block text-xs font-semibold uppercase tracking-wider text-slate-600 mb-2">Alamat Email</label>
                                <input type="email" id="email" name="email" required value="srierma@email.com" class="block w-full px-4 py-3 text-sm text-slate-800 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-gold-500/20 focus:border-gold-500 transition-all">
                            </div>
                            ...
                            <div>
                                ...
                                <input type="tel" id="telepon" name="telepon" required value="081234567890" class="block w-full px-4 py-3 text-sm text-slate-800 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-gold-500/20 focus:border-gold-500 transition-all">
                            </div>
                        </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-card border border-slate-100 p-6 md:p-8 transition-all">
                        ...
                        <div class="space-y-4">
                            <div class="flex justify-between items-center text-sm">
                                ...
                                <span class="font-bold text-slate-800">{{ request('asal', 'Sijunjung') }}</span>
                            </div>
                            <div class="flex justify-between items-center text-sm">
                                <span class="text-slate-400 font-medium">Kota Tujuan</span>
                                <span class="font-bold text-slate-800">{{ request('tujuan', 'Padang') }}</span>
                            </div>
                            <div class="flex justify-between items-center text-sm">
                                <span class="text-slate-400 font-medium">Tanggal Perjalanan</span>
                                <span class="font-bold text-slate-800">{{ request('tanggal', '20 Juli 2026') }}</span>
                            </div>
                            <div class="flex justify-between items-center text-sm">
                                <span class="text-slate-400 font-medium">Jam Keberangkatan</span>
                                <span class="font-bold text-slate-800">{{ request('jam', '05.00 WIB') }}</span>
                            </div>
                            <div class="flex justify-between items-center text-sm">
                                <span class="text-slate-400 font-medium">Armada</span>
                                <span class="font-bold text-slate-800">{{ request('armada', 'Toyota Avanza') }}</span>
                            </div>
                            <div class="flex justify-between items-center text-sm">
                            ...
                            <div class="pt-4 border-t border-slate-100 mt-4 space-y-3">
                                <div class="flex justify-between items-center text-sm">
                                    ...
                                    <span class="font-semibold text-slate-800">Rp 70.000</span>
                                </div>
                                <div class="flex justify-between items-center pt-2 border-t border-dashed border-slate-100">
                                    ...
                                    <span class="text-lg font-black text-gold-600">Rp 70.000</span>
                                </div>
                            </div>

                            <!-- Payment Notice Info Box (UX Correction: Styled info banner instead of plain text note) -->
            ...
            <!-- Form Submit Buttons (Centered bottom) -->
            <div class="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4">
                <a href="/booking/pilih-kursi?kursi={{ request('kursi', '1') }}" class="w-full sm:w-auto px-6 py-3 text-sm font-semibold text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-xl transition-colors text-center cursor-pointer">
                    Kembali
                </a>
